import React from 'react';

export const GemIcon: React.FC<React.SVGProps<SVGSVGElement>> = ({
  className = '',
  ...props
}) => {
  return (
    <svg
      viewBox="0 0 24 30"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={`w-6 h-6 shrink-0 ${className}`}
      {...props}
    >
      <g clipPath="url(#clip0_gem)">
        <path
          d="M3.12035 6.45972C1.80438 7.25391 1 8.67887 1 10.2159V19.8822C1 21.4192 1.80438 22.8442 3.12035 23.6384L9.59501 27.5458C10.9891 28.3872 12.7345 28.3872 14.1287 27.5459L20.6037 23.6384C21.9197 22.8442 22.7241 21.4192 22.7241 19.8821V10.216C22.7241 8.67889 21.9197 7.2539 20.6037 6.45972L14.1287 2.55221C12.7345 1.71089 10.9891 1.71091 9.59501 2.55226L3.12035 6.45972Z"
          fill="#1CB0F6"
          stroke="white"
          strokeWidth="2"
        />
        <path
          fillRule="evenodd"
          clipRule="evenodd"
          d="M10.245 5.45314C10.9646 5.01112 11.8903 5.52889 11.8903 6.37339V9.53852C11.8903 9.91649 11.6927 10.267 11.3693 10.4626L8.42203 12.2455C8.05594 12.4669 7.59363 12.4514 7.24324 12.2058L4.8572 10.5338C4.22077 10.0878 4.24953 9.13584 4.91172 8.7291L10.245 5.45314Z"
          fill="#DDF4FF"
        />
      </g>
      <defs>
        <clipPath id="clip0_gem">
          <rect width="24" height="30" fill="white" />
        </clipPath>
      </defs>
    </svg>
  );
};