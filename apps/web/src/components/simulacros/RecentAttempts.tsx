...
import { BarChart3, CheckCircle, XCircle, Clock, RotateCcw } from 'lucide-react';
    ...
    <div className="px-5 pb-12">
      ...
      <div className="space-y-4">
        {attempts.map(attempt => (
          <div key={attempt.id} className="bg-white rounded-[2rem] border-2 border-slate-100 border-b-[6px] border-b-slate-200 shadow-sm overflow-hidden">
            <div className="px-5 pt-5 pb-3 flex justify-between items-start">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-slate-400 font-bold text-[10px] uppercase">{attempt.date}</span>
                </div>
                <h4 className="font-black text-slate-800 text-[17px] leading-tight">{attempt.title}</h4>
              </div>
              <div className="bg-blue-50 border-2 border-blue-100 px-3 py-2 rounded-2xl flex flex-col items-center">
                <span className="text-[9px] font-black text-blue-400 uppercase mb-0.5">Score</span>
                <span className="text-xl font-black text-blue-600 leading-none">{attempt.score}</span>
              </div>
            </div>
            <div className="px-5 py-3 flex items-center gap-6 border-y border-slate-50 bg-slate-50/50 text-slate-600 font-black text-[12px]">
              <div className="flex items-center gap-1.5">
                <CheckCircle size={14} className="text-emerald-500" /> {attempt.correct} <span className="text-slate-400 text-[9px]">BIEN</span>
              </div>
              <div className="flex items-center gap-1.5">
                <XCircle size={14} className="text-rose-500" /> {attempt.incorrect} <span className="text-slate-400 text-[9px]">MAL</span>
              </div>
              <div className="flex items-center gap-1.5">
                <Clock size={14} className="text-slate-400" /> {attempt.timeUsed}
              </div>
            </div>
