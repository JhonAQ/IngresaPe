...
export const GoalCard: React.FC<GoalCardProps> = ({ career, currentScore, targetScore, isLoaded }) => {
      ...
      <div className="bg-white rounded-[1.8rem] p-4 border-2 border-slate-200 border-b-[6px] border-b-slate-300 shadow-sm relative overflow-hidden">
        <div className="flex items-center justify-between mb-3 relative z-10">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-[19px] font-black text-slate-800 leading-tight">{career}</h2>
          </div>
          <div className="flex flex-col items-end">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Puntaje</span>
            <span className="text-xl font-black text-blue-600 leading-none">{currentScore}</span>
          </div>
        </div>
        ...
        <div className="flex justify-between items-center text-[11px] font-bold">
          <div className="flex items-center gap-1 text-slate-400">
            ...
            Faltan <span className="text-red-500 font-black">{pointsNeeded} pts</span>
          </div>
          <span className="text-slate-400 uppercase tracking-widest">Meta: {targetScore}</span>
        </div>
      </div>
    </div>
  );
