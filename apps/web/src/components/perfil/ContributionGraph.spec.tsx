...
describe('ContributionGraph', () => {
  ...
  it('muestra actividad real en las celdas correspondientes', () => {
    ...
    const active = cells.filter(
      (c) => Number(c.getAttribute('data-intensity')) > 0
    );
  ...
  it('rellena días sin actividad con intensidad 0', () => {
    ...
    const data: HeatmapDay[] = [makeDay(today, 2), makeDay(yesterday, 1)];

    render(<ContributionGraph weeks={4} data={data} />);
    const cells = screen.getAllByTestId('heatmap-day');
    const intensities = cells.map((c) =>
      Number(c.getAttribute('data-intensity'))
    );
  ...
  it('renderiza una cuadrícula vacía cuando no hay actividad', () => {
