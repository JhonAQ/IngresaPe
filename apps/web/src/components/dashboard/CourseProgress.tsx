...
export function CourseProgress({ courseName, progress, iconUrl, onClick }: CourseProgressProps) {
  ...
  const className =
    'relative w-full px-4 py-3 flex items-center justify-between bg-white transition-all text-left rounded-2xl shadow-lg ring-1 ring-black/5';

  if (onClick) {
    return (
      <button onClick={onClick} className={`${className} active:translate-y-[2px] active:shadow-sm`}>
        {body}
      </button>
    );
  }
