...
function useCoursesWithProgress(courses: { id: string; name: string }[]) {
  const utils = trpc.useUtils();

  const topicsQueries = useQueries({
    queries: courses.map((course) => ({
      queryKey: [['content', 'getTopics'], { input: { courseId: course.id } }],
      queryFn: async () => {
        const result = await utils.client.content.getTopics.query({ courseId: course.id });
        return { courseId: course.id, topics: result };
      },
  ...
  const progressByCourse = useMemo(() => {
    ...
    topicsQueries.forEach((q) => {
      if (q.data?.topics?.length) {
        const total = q.data.topics.length;
        const completed = q.data.topics.filter(
          (t: any) => t.userProgress?.isGold || t.userProgress?.isCompleted
        ).length;
        map[q.data.courseId] = total > 0 ? Math.round((completed / total) * 100) : 0;
      } else {
        map[q.data?.courseId ?? ''] = 0;
      }
    });
    return map;
  }, [topicsQueries]);

  const isLoading = topicsQueries.some((q) => q.isLoading);
...
export function CourseSelector({ selectedCourseId, onSelect }: CourseSelectorProps) {
  const { isOpen, close } = useCourseSelector();
  const { data: courses = [], isLoading: isCoursesLoading } = trpc.content.getCourses.useQuery(
    undefined,
    { retry: false, refetchOnWindowFocus: false }
  );
  const { progressByCourse, isLoading: isProgressLoading } = useCoursesWithProgress(
  ...
  return (
    <div className="absolute inset-0 z-50 bg-white flex flex-col">
      ...
      <header className="sticky top-0 z-50 bg-white px-4 pt-4 pb-3 border-b-2 border-duo-border flex items-center gap-4">
        ...
        <h2 className="flex-1 font-black text-[24px] leading-tight tracking-tight text-duo-dark">
          ¿Qué quieres
      ...
      {/* Lista de cursos */}
      <main className="flex-1 overflow-y-auto hide-scrollbar p-5 pb-24">
        {isCoursesLoading || isProgressLoading ? (
          <div className="flex-1 flex items-center justify-center h-full">
            <div className="font-black text-[#58cc02]">Cargando cursos…</div>
          </div>
        ) : coursesData.length === 0 ? (
          <div className="text-center py-12">
            <h3 className="font-black text-[20px] text-[#3c3c3c] mb-2">No hay cursos</h3>
            <p className="text-[#777777]">Vuelve más tarde.</p>
          </div>
        ) : (
