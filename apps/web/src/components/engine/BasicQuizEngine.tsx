...
const quizData = [
  ...
  },
  {
    ...
    correctAnswer: 'c',
    explanation: "¡Atención aquí! La mitocondria es conocida como la 'central energética' de la célula eucariota. En ella ocurre la respiración celular, un proceso vital que toma la glucosa y el oxígeno para convertirlos en ATP (energía pura). ¡Recuérdalo para tu examen!"
  }
];
  ...
  return (
    <div className="w-full max-w-md mx-auto relative bg-[#ffffff] h-[100dvh] flex flex-col font-sans border-x border-[#e5e5e5] overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto hide-scrollbar px-5 pt-6 pb-40 flex flex-col relative z-10">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <h2 className={`font-black text-[24px] text-[#3c3c3c] leading-tight ${question.formula ? 'mb-6' : 'mb-8'}`}>
              {question.prompt}
            </h2>

            {question.formula && (
              <div className="mb-8 w-full bg-[#f7f7f7] border-2 border-[#e5e5e5] rounded-2xl p-6 flex justify-center items-center">
                <span className="font-black text-[32px] text-[#1cb0f6] tracking-wider">{question.formula}</span>
              </div>
            )}
            ...
            <div className="flex flex-col gap-3">
              {question.options.map((opt) => {
                ...
                const baseClasses = "w-full p-4 rounded-2xl border-2 border-b-[4px] text-center transition-all active:border-b-[2px] active:translate-y-[2px] flex items-center justify-center";
                const unselectedClasses = "border-[#e5e5e5] border-b-[#e5e5e5] bg-white hover:bg-slate-50";
                const selectedClasses = "border-[#84d8ff] border-b-[#1899d6] bg-[#ddf4ff]";
                
                const disabledClass = status === 'checked' ? 'pointer-events-none opacity-60' : 'cursor-pointer';
                const finalOpacity = (status === 'checked' && isSelected) ? 'opacity-100' : disabledClass;
                ...
                return (
                  <motion.button
                    ...
                    className={`${baseClasses} ${isSelected ? selectedClasses : unselectedClasses} ${finalOpacity}`}
                  >
                    <span className={`font-bold text-[18px] ${isSelected ? 'text-[#1cb0f6]' : 'text-[#3c3c3c]'}`}>
                      {opt.text}
                    </span>
                  </motion.button>
                );
      ...
      <FeedbackDrawer 
        ...
        correctAnswerText={question.options.find(o => o.id === question.correctAnswer)?.text}
        onCheck={handleCheck}
