...
export const ChunkyHeart = ({ className = "w-6 h-6" }) => (
  <svg viewBox="0 0 100 100" className={`drop-shadow-sm ${className}`}>
    <path d="M 50 90 L 15 50 C 5 40 5 20 20 10 C 30 5 40 10 50 20 C 60 10 70 5 80 10 C 95 20 95 40 85 50 Z" fill="#ff4b4b" />
    <path d="M 22 15 C 30 8 42 12 48 20" stroke="#ffffff" strokeWidth="5" strokeLinecap="round" fill="none" opacity="0.5"/>
  </svg>
);

export const DuoBot = ({ className = "w-24 h-24", state = "idle" }: { className?: string, state?: string }) => {
  return (
    <svg viewBox="0 0 100 100" className={`drop-shadow-md ${className}`}>
      {/* Antena */}
      <rect x="46" y="5" width="8" height="20" rx="4" fill="#a568cc" />
      <motion.circle 
        cx="50" cy="5" r="8" fill="#ce82ff" 
        animate={state === 'thinking' ? { fill: ['#ce82ff', '#ffc800', '#ce82ff'], scale: [1, 1.2, 1] } : {}}
      ...
      <path d="M 25 45 C 25 38 30 35 35 35 L 75 35 L 55 65 L 25 65 Z" fill="#ffffff" opacity="0.1"/>
      
      {/* Ojos Dinámicos */}
      {state === 'thinking' ? (
        <g>
          <motion.circle cx="35" cy="48" r="5" fill="#ffc800" 
            animate={{ scale: [1, 0.5, 1], opacity: [1, 0.5, 1] }} transition={{ repeat: Infinity, duration: 0.8 }} />
          <motion.circle cx="65" cy="48" r="5" fill="#ffc800" 
            animate={{ scale: [0.5, 1, 0.5], opacity: [0.5, 1, 0.5] }} transition={{ repeat: Infinity, duration: 0.8 }} />
        </g>
      ) : state === 'happy' ? (
        <g>
          <path d="M 30 50 Q 35 40 40 50" stroke="#58cc02" strokeWidth="5" strokeLinecap="round" fill="none" />
          <path d="M 60 50 Q 65 40 70 50" stroke="#58cc02" strokeWidth="5" strokeLinecap="round" fill="none" />
        </g>
      ) : (
        <g>
      ...
      <path d="M 22 30 L 40 30" stroke="#ffffff" strokeWidth="4" strokeLinecap="round" opacity="0.4" />
    </svg>
  );
};
...
export function EngineHeader({ progress, lives, onClose }: EngineHeaderProps) {
  return (
    <header className="flex items-center justify-between px-4 pt-6 pb-2 bg-white z-20 shrink-0 gap-4">
      <button onClick={onClose} className="text-[#afafaf] hover:text-[#777777] transition-colors active:scale-95">
...
export function FeedbackDrawer({
  status, isCorrect, isCheckDisabled, correctAnswerText, onCheck, onContinue, onOpenAI
}: FeedbackDrawerProps) {
  return (
        ...
        {status === 'checked' && isCorrect && (
          ...
          >
            ...
            <div className="flex items-center gap-3 mb-5">
              ...
              <h3 className="font-black text-[#58a700] text-[22px]">¡Excelente!</h3>
            </div>
          ...
          >
            ...
            <div className="flex flex-col mb-4">
              <div className="flex items-center gap-3 mb-2">
                ...
                <h3 className="font-black text-[#ea2b2b] text-[22px]">¡Incorrecto!</h3>
              </div>
              <div className="pl-11">
                <p className="font-bold text-[#ea2b2b] text-[15px]">Solución correcta:</p>
...
export function DuoMaxModal({ isOpen, onClose, trick, explanation }: DuoMaxModalProps) {
          ...
          >
            <div className="flex justify-between items-center mb-6">
              <div className="flex items-center gap-2">
                ...
                <span className="font-black text-white text-[18px]">Duo Max</span>
              </div>
              <button 
                onClick={onClose} 
            ...
            <div className="flex items-end gap-3 mb-6">
              <div className="shrink-0 relative">
                ...
                <DuoBot className="w-[72px] h-[72px] relative z-10" state={aiState} />
              </div>
                ...
                {(aiState === 'typing' || aiState === 'finished') && (
                  <div className="flex flex-col items-start">
                    ...
                    {trick && (
                      ...
                      >
                        <Sparkles size={16} className="text-[#ffc800] fill-[#ffc800]" />
                        <span className="font-black text-[#ffc800] text-[13px] uppercase tracking-widest">{trick}</span>
                      </motion.div>
                    )}
